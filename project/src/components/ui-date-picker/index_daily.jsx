...
class UIDatePicker extends Component{
	...
	getVal(){
		...
		if(isEmptyObject(this.props.value[0]) || !this.props.value[0]){
			res[0] = null;
		}else{
			res[0] = this.props.value[0];
		}

		if(isEmptyObject(this.props.value[1]) || !this.props.value[1]){
			res[1] = null;
		}else{
			res[1] = this.props.value[1];
		}

		if(res[0] && res[1])return {value : res}
	}
  render(){
