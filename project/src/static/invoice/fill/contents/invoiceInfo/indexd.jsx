...
class InvoiceInfo extends Component {
	...
	componentWillMount() {
		...
		this.setState({
			billHeaderType: this.props.main.detail.invoiceInfo.billHeaderType,
			// billHeader: this.props.main.detail.invoiceInfo.billHeader
		});
	}
	...
	_changeInvoiceInfo(name, ele) {
		...
		switch (name) {
			case 'billHeader':
				...
				break;
			case 'billHeaderType':
				let orignBillHeaderType = this.invoiceInfo.billHeaderType;
				value = ele.target.value;
				data.billHeaderType = value;

				debugger
				// 切换默认值
				if(value == orignBillHeaderType) {
					data.billHeader = this.invoiceInfo.billHeader;
				} else {
					let info =this.props.header.info.agentInfo;
					data.billHeader = (value == 1 ? info.agentName : info.agentUserName);
				}
			...
			<div className="form-block">
				...
				<dl>
					...
					<dd className="bill-header-wrap">
						...
						reg={/^[^\s][^]+$/}
						sign='发票抬头'
						value={this.props.main.invoiceInfo.billHeader}
						replaceValue={this.props.main.invoiceInfo.billHeader}
						placeholder='发票抬头'/>
					</dd>
				...
				<dl>
					...
					<dd className="bill-header-type-wrap">
						...
						id='bill-header-type' 
						className='flex1-box' 
						value={this.props.main.invoiceInfo.billHeaderType} 
				...
				<dl>
					...
					<dd className="bill-content-wrap">
						...
						<Select
		                    ...
		                    value={`${this.props.main.invoiceInfo.billContent}`}
		                >
				...
				<dl>
					...
					<dd className="bill-type-wrap">
						<del className='fixed'>*</del>
						<RadioGroup id='bill-type' 
						className='flex1-box' 
						value={this.props.main.invoiceInfo.billType} 
						onChange={this._changeInvoiceInfo.bind(this, 'billType')}>
				...
				<dl>
					...
					</dt>
					{
						...
						<dd className="bill-tax-wrap">
							...
							sign='纳税人识别号'
							value={this.props.main.invoiceInfo.billTaxpayerNum}
							placeholder='纳税人识别号'/>
						</dd>:null
					}
					{
				...
				<dl>
					...
					<dd>
						...
						name='remark'
						value={this.props.main.invoiceInfo.remark}
						/>
					</dd>
				</dl>
				<dl>
					...
					&yen; {this.props.main.detail.totalInfo.billAmount}
					</dd>
				</dl>
			</div>
		)
