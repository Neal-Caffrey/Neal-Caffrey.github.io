...
import {updateDropoffVo} from '../../action/dailyAction.js'
...
class DropOffModal extends Component{
  ...
  onSelectAir(selObj){
    ...
    this.props.dispatch(updateDropoffVo(object));
  }
  ...
  clickOn(){
    let obj =  {
      ...
      time : this.data.time,
      isCai : this.props.isShowLastTime
    };
  ...
  renderTime(){
    // let time =
    let time = <div className='form-fill-control flex-box pickup-model-input'>
      <lable htmlFor='pickup-place'>用车时间</lable>
    ...
    // if(this.props.isDaily && this.props.isShowLastTime){
    //   return time;
    // }
    return null
  }
  renderPortSection(){
    return (
      <div className='w-pickup-wrap' style={{display:this.state.byAirCode ? 'none' : 'block'}}>
        ...
        {
          !this.props.isDaily && <div className='form-fill-control flex-box pickup-model-input'>
...
function mapStateToProps(state) {
  ...
  const {currentDayIndex,currentObject,planList} = leftSide;
  ...
  let {dropoffVo} = currentObject.dailyVo;
  let {dailyDropoffVo,isShowDropoff,pickUpModelKey} = daily || {};
  let islast = planList.length-1 === currentDayIndex;
  // if(isDaily){
  ...
  // }
  // if(isDaily === true){
  //   dailyDropoffVo = dailyDropoffVo || {};
  // }else{
  //   dailyDropoffVo = dropoffVo || {};
  // }
  dailyDropoffVo = dailyDropoffVo || dropoffVo || {};
  let arrCityId = (dailyDropoffVo.flightVo && dailyDropoffVo.flightVo.arrCityId) || (dailyDropoffVo.flightVo && dailyDropoffVo.flightVo.cityId) || -1;
