...
let Sea =  OnclickOutside(class SearchNearCity extends Component {
  ...
  getFilterArr(keyword){
    ...
    return this.state.hotList.filter((item) => {
      return reg.test([item.cityName,item.countryName,item.spell,item.cityEnName || '',item.enName || ''].join(','));
    });
  }
  getHotCitys(){
    ...
    let opt = {
      ...
      successHandle: (res) => {
          this._navSetting = getNavSetting(res.data);
          this.setState({
            hotList : res.data,
            isLoading : false,
            hotRending : true,
  ...
  _makeFilterReplace(obj){
    let str = `${obj.cityName}  ${obj.cityEnName || obj.enName || ''}-${obj.countryName}`
