    ...
    return {
      clickFilter : (filter,tabIndex)=>{
        ...
        let carLength = (this.props.carList.quoteInfos.filter((val) => {
          let operateStr = `${val.seatType} ${filter}`;
      ...
      goToOrder : (quoteInfo)=>{
        ...
        this.setFill(fillData,key,(res)=>{
          if(res.data === 200){
            // window.open('./orderFill.html?key='+key)
            tab.location = './orderFill.html?key='+key;
          }else{
            tab.close();
          }
            ...
            <ul className="cars">
              {
                (()=>{
                  ...
                  let cars =  this.data.carList.map((val,index)=>{
                    ...
                    let operateStr = `${val.seatType} ${this.state.filter}`;
                    ...
                    return (
                      <li key={index} className="car">
                        ...
                        <div className="btn" onClick={this.bindEvent().goToOrder.bind(this,val)}>预订</div>
                        <div className="priceBox">
                          {
                            val.urgentFlag == 1 ? 
                            <div className="urgent-box">
                              <span>急</span>
                              <div className="tips">预定期距离服务时间过短，会影响价格的浮动</div>
                            </div> : null
                          }
                          <div className="price">¥{val.priceWithAddition}</div>
                          <div className="pricemsg"><p>该价格有效期至</p><p>{val.payDeadline}</p></div>
                        </div>
