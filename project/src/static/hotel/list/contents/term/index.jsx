...
class UIHotelTerm extends Component{
  ...
  chosePrice(key, checked, event){
    let _priceChoseList = this.priceChoseList;
    let states = {
      priceStatus : key,
    };
    Object.assign(states, {
      priceChoseList : this.setChoseOne(key, _priceChoseList, checked),
    })
    this.setState(states);
    this.choseResult();
  }

  choseStar(key, checked, event){
    let tag = event.target;
    let _starChoseList = this.starChoseList;
    let states = {
      starStatus : key,
     ...
     Object.assign(states, {
      starChoseList : this.setChose(key, _starChoseList, checked),
    });
    this.setState(states);
    this.choseResult();
  }
            ...
            onClick={this.clearStar.bind(this)}><em>不限</em></dd>
            {
              this.starChoseList.map((item, key) => {
                return (
                  ...
                  text={item.text}
                  name={'star' + key}
                  onHandle={this.choseStar.bind(this, key)}
              ...
              this.priceChoseList.map((item, key) => {
                return (
                  <dd key={key}>
                    ...
                    text={item.text}
                    name={'price' + key}
                    onHandle={this.chosePrice.bind(this, key)}
                    checked={item.checked} />
                  </dd>)
              })
            }
